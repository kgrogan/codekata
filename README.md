...
ReactQ is a minimalist API for managing events, subscribers (listeners), and event handlers. ReactQ, when integrated
into your application, allows you to declare that an event has happened. To notify a listener and activate all of its
associated handlers an event must be "triggered". There is no requirement for an event to have any listeners. An event
may have from 0 to "unlimited" subscribed listeners. 

Listeners are called (and their handlers executed) in subscription order by default; however, this can be controlled by
providing an explicit numeric priority when subscribing the listener. Higher numbers provide a higher priority. Higher
priority listeners are called before their lower priority counterparts. For example, a listener with a priority of 10
will be executed before an event handler with a priority of 5, even if the latter was subscribed first.

A listener is a standard "callable"; it's executed when an event is "triggered".
...
**Subscribing a listener to the 'article.published' event and attaching a lambda as a callback.**

    $react = new ReactQueue();
    $react->on('article.published')->call(function() {});
